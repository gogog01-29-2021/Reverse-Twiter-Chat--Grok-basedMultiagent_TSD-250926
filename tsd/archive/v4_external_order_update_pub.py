...
from archive.v4_streaming import WebsocketPublisher, ZmqPubTransport
from dsm.utils.conversion_utils import iso_to_epoch_ms, now_epoch_ms

logging.basicConfig(format="%(asctime)s - %(levelname)s - %(message)s", level=logging.INFO)


class ExternalOrderUpdatePublisher(WebsocketPublisher):
    """
    Abstract base class to stream and publish external order updates over ZMQ.
    """

    def __init__(self, ws_url: str, transport: ZmqPubTransport, api_key: str, secret_key: str,
                 verbose: bool = False, debug: bool = False, log_on_change: bool = False):
        super().__init__(ws_url, transport, verbose, debug, log_on_change)
    ...
    def on_message(self, ws, message):
        ...
        try:
            ...
            if isinstance(data, dict):
                self._handle_order_update(data, now_epoch_ms())
        except Exception:
    ...
    def _subscribe(self, ws):
        raise NotImplementedError("Must implement _subscribe in concrete subclass")

    def _handle_order_update(self, data: Dict[str, Any], time_received: int):
        raise NotImplementedError("Must implement _handle_order_update in concrete subclass")


class CoinbaseExternalOrderUpdatePublisher(ExternalOrderUpdatePublisher):
    """
    Coinbase-specific external order update publisher.
    """

    EXCHANGE = Exchange.COINBASE
    WS_URL = TransportAddressBuilder.websocket(EXCHANGE)

    def __init__(self, transport: ZmqPubTransport, api_key: str, secret_key: str,
                 verbose: bool = False, debug: bool = False, log_on_change: bool = False):
        super().__init__(self.WS_URL, transport, api_key, secret_key, verbose, debug, log_on_change)
...
if __name__ == "__main__":
    coinbase = CoinbaseExternalOrderUpdatePublisher(
        transport=ZmqPubTransport(
            zmq_bind_addr=TransportAddressBuilder.external_order_update(Exchange.COINBASE, "tcp://0.0.0.0"), debug=True),
