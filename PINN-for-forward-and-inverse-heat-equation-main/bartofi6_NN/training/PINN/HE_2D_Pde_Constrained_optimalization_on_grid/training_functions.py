...
def create_q_nn(training_setting_dictionary: dict, data: dict):
    ...
    for layer_index in range(training_setting_dictionary["q_nn_num_of_layers"]):
        ...
        q_nn.add(q_nn_layer)
    q_nn.add(tf.keras.layers.Dense(1, kernel_initializer=training_setting_dictionary["q_nn_kernel_initializer"], dtype=dtype))

    data["q_nn"] = q_nn


def _call_train_u_nn_and_q_nn_function_with_one_parameter_difference_for_grid_domain(training_setting_dictionary: dict, data: dict, train_only_u_nn: bool):
    return train_u_nn_and_q_nn(data["u_nn"],
                               data["q_nn"],
                               data["equation_rhs_function"],
                               data["initial_condition_function"],
                               data["desired_function_at_final_time"],
                               data["heat_coef"],
                               data["alpha"],
                               data["t_start"],
                               data["t_stop"],
                               training_setting_dictionary["num_of_t_training_points"],
                               data["x_start"],
                               data["x_stop"],
                               training_setting_dictionary["num_of_x_training_points"],
                               data["y_start"],
                               data["y_stop"],
                               training_setting_dictionary["num_of_y_training_points"],
                               training_setting_dictionary["num_of_batches"],
                               training_setting_dictionary["initial_epoch"],
                               training_setting_dictionary["num_of_epochs"],
                               training_setting_dictionary["write_loss_values_every_x_epochs"],
                               training_setting_dictionary["boundary_loss_coef"],
                               training_setting_dictionary["init_cond_loss_coef"],
                               training_setting_dictionary["cost_functional_loss_coef"],
                               training_setting_dictionary["optimizer"],
                               training_setting_dictionary["loss_fn"],
                               callbacks=None,
...
def train_model_on_grid_domain(training_setting_dictionary: dict, data: dict):
    ...
    _, _, training_time_of_u_nn_and_q_nn = \
        _call_train_u_nn_and_q_nn_function_with_one_parameter_difference_for_grid_domain(training_setting_dictionary,
                                                                                         data,
...
def save_model(training_setting_dictionary: dict, data: dict):
    ...
    u_nn_save_file = os.path.join(save_dir_for_current_specific_training_setting, f"u_nn.weights.h5")
    data["u_nn"].save_weights(u_nn_save_file)

    q_nn_save_file = os.path.join(save_dir_for_current_specific_training_setting, f"q_nn.weights.h5")
